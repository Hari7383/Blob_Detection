...
while True:
    ...
    thresh_with_blobs = cv2.cvtColor(thresh, cv2.COLOR_GRAY2BGR)  # Convert to BGR for colored drawing
    cv2.drawContours(thresh_with_blobs, small_contours, -1, (0, 255, 0), 2)  # Green for small blobs
    cv2.drawContours(thresh_with_blobs, big_contours, -1, (0, 0, 255), 2)  # Red for big blobs
   ...
   # cv2.imshow("Original Frame", frame)
    cv2.imshow("Threshold", thresh_with_blobs)  # Show blobs on the thresholded image
   # cv2.imshow("Blob Morphology", blob)
